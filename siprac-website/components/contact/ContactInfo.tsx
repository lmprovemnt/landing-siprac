  ...
  const infoCards = [
    {
      title: "Teléfonos",
      icon: "fas fa-phone",
      items: ["311 630 0848", "315 437 6818", "315 865 3629"],
    ...
    },
    {
      ...
      content:
        "Comuníquese directamente por WhatsApp para una respuesta rápida",
      button: {
              ...
              {card.items && (
                <div className="space-y-4 w-full mb-8">
                  {card.items.map((item, i) => (
                    <div
                      key={i}
                      className="py-3 px-4 md:py-4 md:px-6 rounded-2xl text-[10px] sm:text-xs md:text-[13px] text-gray-700 font-bold uppercase tracking-widest border border-white/5 opacity-80 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 md:gap-4"
                    >
                      {card.title === "Teléfonos" && (
                        <PhoneForwarded className="size-3 md:size-4 text-orange-500 shrink-0" />
                      )}
                      {card.title === "Correos Electrónicos" && (
                        <Send className="size-3 md:size-4 text-orange-500 shrink-0" />
                      )}
              ...
              {card.details && (
                <div className="space-y-5 w-full mb-8 text-center">
                  {card.details.map((detail, i) => (
                    <div
                      key={i}
                      className="flex flex-col items-center justify-center bg-[#FAF9F6] p-5 rounded-xl border border-white/5 gap-2"
                    >
